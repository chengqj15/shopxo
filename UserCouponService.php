...
class UserCouponService
{
    ...
    public static function CouponNoticeUserList($params)
    {
        // 用户优惠劵过期处理
        self::CouponUserExpireHandle();
        $where = ['c.is_use' => 0, 'c.is_expire' => 0, 'c.is_valid' => 1];
        if(isset($params['coupon_id'])){
           $where['c.coupon_id'] = $params['coupon_id']; 
        }
        if(isset($params['id'])){
           $where['c.id'] = $params['id']; 
        }

        $data = Db::name('PluginsCouponUser')->alias('c')->join(['__USER__'=>'u'], 'c.user_id=u.id')->field('c.time_end, u.weixin_openid')->where($where)->select();
        return $data;
    }
