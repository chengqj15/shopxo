...
class CouponService
{
    ...
    public static function CouponSend($params = [])
    {
        ...
        switch($coupon['expire_type'])
        {
            ...
            // 固定日期
            case 1 :
                if(date('Y-m-d', $coupon['fixed_time_end']) < date('Y-m-d', time()))
                {
        ...
        switch($coupon['expire_type'])
        {
            ...
            // 固定日期
            case 1 :
                $time_start = strtotime(date('Y-m-d', $coupon['fixed_time_start']));
                $time_end = strtotime(date('Y-m-d', $coupon['fixed_time_end'])) + (86400-1);
                break;
        }
    ...
    public static function CouponExpireNotice($params = [])
    {
        if(isset($params['coupon_id'])){
            $coupon = Db::name('PluginsCoupon')->find(intval($params['coupon_id']));
            if(empty($coupon)){
                return DataReturn('无效的优惠券', -1); 
            }
        }else if(isset($params['id'])){
            $data = Db::name('PluginsCouponUser')->where(['id' => $params['id']])->find();
            if(empty($data)){
                return DataReturn('无效的优惠券', -1); 
            }
            $coupon = Db::name('PluginsCoupon')->find(intval($data['coupon_id']));
            if(empty($coupon)){
                return DataReturn('无效的优惠券', -1); 
            }
        }else{
            return DataReturn('通知发送失败，无效参数');
        }
        
        $data = UserCouponService::CouponNoticeUserList($params);
        $suc_count = 0;
        if(!empty($data)){
            foreach($data as $v)
            {
                $notice_param = [
                'touser' => $v['weixin_openid'],
                'template_id' => 'ECOwsadqyExeD0jXSWuUH8YXIrr4eQqVcoqEdRG0Z14',
            ...
            //Log::write('CouponExpireNotice ret:' . json_encode($result));
            if($result['code'] == 0){
                $suc_count++;
            }
        }

        return DataReturn('成功发送通知:'. $suc_count . ' 条', 0);
    }
}
?>
